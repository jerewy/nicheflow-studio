...
Open an Instagram Reel and click the NicheFlow extension icon. This opens the
docked **side panel**, which stays open while you click around Instagram (a normal
popup would close the moment you click the page). Choose a pool, optionally pin an
account, and click **Queue Current**. Repeat on additional Reels, then click
**Process Queue**. NicheFlow sends the queued URLs to Apify in one batch and
processes it in the background — you can keep queueing more Reels while it runs;
only the items in that batch are cleared on completion. The badge shows the queued
count and the panel shows pool size, estimated Apify spend, and the last batch
result.
